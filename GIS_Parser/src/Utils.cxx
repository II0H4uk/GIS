...
#include <ctime>
#include <fstream>
#include <iomanip> // for format time
#include <iostream>
#include <string>
#include <sstream>
...
Utils::Subcircuit Circuits::Utils::SpiceParser::ExtractSubcircuit(const std::string& fileName){
    ...
    if (file.is_open()) {
        ...
        while (getline(file, line)) {
            ...
            // Simple check for directive ".SUBCKT"
            if (line.substr(0, 7) == ".SUBCKT"){
                subcircuit= Circuits::Utils::SpiceParser::parseSubcircuitLine(line);
                continue;
            }
            if (line == ".ENDS") break;
...
Utils::Subcircuit Circuits::Utils::SpiceParser::parseSubcircuitLine(const std::string& line) {
    Subcircuit subcircuit;
    ...
    std::getline(ss, subcircuit.name, ' ');
...
Utils::TopologyComponent Circuits::Utils::SpiceParser::parseComponent(const std::string& line)
{
